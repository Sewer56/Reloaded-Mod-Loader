...
namespace Reloaded.Process.DllInjector
{
    ...
    {
        /// <summary>
        /// A handle to the Kernel32 Module, whereby LoadLibraryW and other functions are stored.
        /// </summary>
        private Kernel32.SafeLibraryHandle _kernel32Handle;
        ...
        /// <summary>
        /// The destination to which we will write the parameters for our function to.
        /// </summary>
        ...
        /// Once the DLL Injector is initialized, DLL injection may be performed by calling InjectDLL();
        /// </summary>
        /// <param name="process">
        /// The process object that is to be used. 
        /// The handle can be obtained by Process.Handle or found via various other means.
        /// </param>
        public DllInjector(System.Diagnostics.Process process)
        {
            // Set the Location and Handle to the Process to be Injected.
            _process = process;
            _memorySource = new ExternalMemory(process);
            ...
            _kernel32Handle = Kernel32.LoadLibrary("kernel32");
            ...
            _loadLibraryAddress = Kernel32.GetProcAddress(_kernel32Handle, "LoadLibraryW");
        }
        ...
        public void InjectDll(string libraryPath, IntPtr parameterPointer, string functionToExecute)
        {
            ...
            IntPtr moduleBaseAddress = (IntPtr)RemoteDllFunctions.CallLibrary(_process.Handle, _loadLibraryAddress, libraryNameMemoryAddress);
            ...
            IntPtr mainFunctionOffset = LocalDllFunctions.GetFunctionOffset(libraryPath, functionToExecute);
            ...
            RemoteDllFunctions.CallLibrary(_process.Handle, functionAddress, parameterPointer);
        }
        ...
        private IntPtr WriteModuleNameBytes(string libraryPath)
        {
            ...
            IntPtr processPointer = _memorySource.Allocate(libraryNameBytes.Length);

            // Write the name of the library to the memory space of the process.
            _memorySource.WriteRaw(processPointer, libraryNameBytes);
